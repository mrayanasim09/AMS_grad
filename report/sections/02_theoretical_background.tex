...
\end{equation}
\textbf{The Fatal Flaw in \textsc{Adam}:} Because \textsc{Adam} computes $v_t$ via an exponential moving average, $v_t$ can shrink whenever a large gradient is followed by smaller gradients. When $v_t < v_{t-1}$, $\Gamma_t$ has negative diagonal entries. This invalidates the telescoping bound (the quadratic sum in Eq.~\eqref{eq:telescoping_sum} can no longer be bounded by $D_\infty^2 \text{Tr}(V_T - V_1)$), breaking the standard OCO regret proof. The actual linear regret $\Omega(T)$ and dynamic divergence to the suboptimal boundary are established by the concrete counterexample dynamics.

\subsection{The AMSGrad Resolution}
\citet{reddi2018convergence} proved that convergence is restored if the effective second moment is guaranteed to be coordinate-wise non-decreasing. \textsc{AMSGrad} enforces this via the running maximum on the raw second-moment estimate:
\begin{equation}
    \hat{v}_t = \max\left(\hat{v}_{t-1}, v_t\right), \qquad \hat{v}_0 = \mathbf{0},
    \label{eq:amsgrad_max}
\end{equation}
following Algorithm~2 of \citet{reddi2018convergence} without second-moment debiasing. Since $\hat{v}_t \ge \hat{v}_{t-1}$, for any non-increasing step-size schedule $\alpha_t = \frac{\alpha}{\sqrt{t}}$:
\begin{equation}
