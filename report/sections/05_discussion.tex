...
\subsection{Theoretical Guarantees vs. Practical Conservatism}
...
However, our mechanism discrimination probes illuminate why \textsc{AMSGrad} does not consistently outperform \textsc{Adam} on real-world deep neural network architectures:
\begin{enumerate}
    ...
    \item \textbf{Travel-Time Limitations at Large $C$:} While \textsc{AMSGrad} and \textsc{AdaGrad} are certified non-divergent everywhere, their displacement rate scales inversely with $\sqrt{\hat{v}} \approx C$. Under sparse bursts ($C=1000$), \textsc{AMSGrad} requires substantially more iterations to traverse large parameter distances compared to adaptive methods that allow second moments to adapt dynamically.
\end{enumerate}
...
In this study, we presented an end-to-end, reproducible investigation of the \textsc{Adam} $\to$ \textsc{AMSGrad} transition. We successfully reconstructed both the deterministic and stochastic counterexamples of \citet{reddi2018convergence}, established direct visual tracking of the $\Gamma_t \succeq 0$ OCO breakdown, and performed a pre-registered phase-boundary analysis across $(\beta_2, C)$ space. Our findings refute the simple $C^{-1}$ memory-horizon scaling law, revealing that for large burst spacing ($C \ge 100$), \textsc{Adam}'s divergence becomes $\beta_2$-independent and is driven by post-burst first-moment drift.
